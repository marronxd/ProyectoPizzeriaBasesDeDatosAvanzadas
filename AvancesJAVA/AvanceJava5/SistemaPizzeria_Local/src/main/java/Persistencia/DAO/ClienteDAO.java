...
public class ClienteDAO implements IClienteDAO {
    ...
    private Cliente extraerCliente(ResultSet rs) throws SQLException {
        ...
        cliente.setEstado(estadoStr.equalsIgnoreCase("Activo") ? EstadoCliente.ACTIVO : EstadoCliente.INACTIVO);
    ...
    public Cliente agregarCliente(Cliente cliente, Domicilio domicilio, Telefono telefono) throws PersistenciaException{
        ...
        try (Connection conn = this.conexionBD.crearConexion(); CallableStatement cs = conn.prepareCall(comando)) {
            ...
            if (fecha == null) {
                LOG.log(Level.SEVERE, "La fecha de nacimiento no puede ser nula");
                throw new PersistenciaException("La fecha de nacimiento no puede ser nula");
            }
            cs.setDate(7, java.sql.Date.valueOf(fecha));
            
            //Asigna valores a los parámetros del domicilio
            if (domicilio == null) {
                LOG.log(Level.SEVERE, "El domicilio no puede ser nulo");
                throw new PersistenciaException("El domicilio no puede ser nulo");
            }
            ...
            if (telefono == null) {
                LOG.log(Level.SEVERE, "El teléfono no puede ser nulo");
                throw new PersistenciaException("El teléfono no puede ser nulo");
            }
            ...
            LOG.log(Level.INFO, "Cliente insertado con éxito. ID: {0}", cliente.getId());
            return cliente;
    ...
    public Cliente actualizarCliente(Cliente cliente) throws PersistenciaException {
        ...
        try (Connection conn = this.conexionBD.crearConexion(); PreparedStatement ps = conn.prepareStatement(comando)) {
            ...
            String estadoDB = cliente.getEstado().name().equalsIgnoreCase("ACTIVO") ? "Activo" : "Inactivo";
            ...
            //Regresa el cliente
            LOG.log(Level.INFO, CLASE + " actualizado con éxito. ID: {0}", cliente.getId());
            return cliente;
    ...
    public boolean darBajaCliente(int idCliente) throws PersistenciaException {
        ...
        try (Connection conn = this.conexionBD.crearConexion(); PreparedStatement ps = conn.prepareStatement(comando)) {
            ...
            //Asigna cada valor del domicilio ("?") una posición en la consulta
            ps.setString(1, EstadoCliente.INACTIVO.getValorBD());
            ps.setInt(2, idCliente);
            ps.setString(3, EstadoCliente.ACTIVO.getValorBD());
            
            //Valida si se afectó una fila
